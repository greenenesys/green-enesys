...
const renderFilterItems = (items, activeItems, onChange) => {
    ...
    return items.map(item =>
        <FilterItem key={item}>
            <Paragraph strip> { toUpperCase(item) } </Paragraph>
            <Checkbox checked={activeItems.includes(item)} onChange={() => onChange(item)}/>
        </FilterItem>
    )
}
...
class Filter extends React.Component {
    ...
    get statusString () {
        ...
        return activeStatus.map(status => toUpperCase(status)).toString().replace(',', ', ')
    }
        ...
        return (
            <SidebarItemWrapper>
                <Left>
                    ...
                    <Paragraph fontWeight='medium' strip mt={-1}> {status.length === activeStatus.length ? 'All' : this.statusString} </Paragraph>
                </Left>
            
                <div>
                    <IconWrapper active={this.state.show} onClick={this.handleClick} style={{ cursor: 'pointer' }}>
                        <svg width="18px" height="14px" viewBox="0 0 18 14" version="1.1">
                            <g id="Artboard" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                                <path d="M0,0 L18,0 L18,2 L0,2 L0,0 Z M4,6 L18,6 L18,8 L4,8 L4,6 Z M8,12 L18,12 L18,14 L8,14 L8,12 Z" id="Combined-Shape" fill="#FFC539" fill-rule="nonzero" />
                            </g>
                        </svg>
                    </IconWrapper>
